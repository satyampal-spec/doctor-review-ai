...
function polishManualSpicesReview(rawText, liked, shopName) {
  ...
  const name = shopName || 'this brand';
...
export default function SpicesReviewPage({ params }) {
  const { shopId } = params;
  ...
  const googleUrl =
    ...
    `https://www.google.com/search?q=${encodeURIComponent((shop?.shop_name || '') + ' reviews')}`;
  ...
  const handleGenerate = async () => {
    ...
    const result = generateShopReview({
      shopName: shop?.shop_name || 'This brand',
  ...
  const handlePolish = async () => {
    ...
    const polished = polishManualSpicesReview(manualText, liked, shop?.shop_name);
  ...
  const regenerate = () => {
    if (mode === 'manual') {
      setPolishedReview(polishManualSpicesReview(manualText, liked, shop?.shop_name));
    } else {
      const result = generateShopReview({
        shopName: shop?.shop_name || 'This brand',
        ownerName: shop?.owner_name || '',
  ...
  if (notFound) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fdf6ec' }}>
      <div style={{ textAlign: 'center', padding: 32 }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 700, color: '#111' }}>Brand not found</h2>
      ...
      <div style={{ background: THEME.gradient, padding: '0 20px' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', padding: '28px 0 32px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.18)', borderRadius: 100, padding: '5px 14px', fontSize: 12, color: 'rgba(255,255,255,0.92)', fontWeight: 600, maxWidth: '100%' }}>
            ...
            <span style={{ minWidth: 0, overflowWrap: 'break-word' }}>Verified Brand{shop.location ? ` · ${shop.location}` : ''}</span>
          </div>
        </div>
      </div>
        ...
        {step === 3 && (
          <div className="anim-up review-card" style={{ background: '#fff', borderRadius: 24, padding: 28, boxShadow: '0 8px 40px rgba(192,57,43,0.1)', border: '1px solid #f5e3d0' }}>
              ...
              {DURATION_OPTIONS.map((opt) => {
                ...
                return (
                  <button key={opt.key} onClick={() => setDuration(opt.key)}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderRadius: 14, border: `2px solid ${sel ? THEME.primary : '#e2e8f0'}`, background: sel ? THEME.light : '#fafafa', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'left', fontSize: 14, fontWeight: sel ? 700 : 500, color: sel ? THEME.dark : '#374151' }}>
                    <span>{opt.label}</span>
            ...
            <button onClick={() => setStep(4)} disabled={!duration}
              style={{ width: '100%', padding: '15px', borderRadius: 14, background: duration ? THEME.gradient : '#e2e8f0', color: '#fff', border: 'none', fontWeight: 700, fontSize: 15, cursor: duration ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
              Continue →
            </button>
          </div>
        )}
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <button onClick={() => setMode('auto')}
                    style={{ padding: '20px 22px', borderRadius: 18, border: '2px solid #f5e3d0', background: THEME.light, cursor: 'pointer', textAlign: 'left', transition: 'all 0.18s' }}>
