...
const ZOFF_ID = 'zoff-foods-official';
const ZOFF_FALLBACK = {
  id: ZOFF_ID,
  shop_name: 'ZOFF Foods',
  owner_name: '',
...
const CONFIG = CATEGORY_CONFIG.spices;

const THEME = {
  primary: '#c0392b',
  dark:    '#96311f',
  gradient:'linear-gradient(135deg,#c0392b 0%,#e8792b 100%)',
  light:   '#fdf6ec',
  ring:    '#f5cba7',
};
...
const LIKED_PHRASES = {
  ...
  value:      'the good value for the price',
  delivery:   'the fast delivery on Zepto/Blinkit',
  clean:      'that there were no stones, husk, or dust in the pack',
  cumin:      'how fresh and aromatic the cumin (jeera) was',
  soya:       'how good the soya chunks turned out in texture and taste',
  clove:      'how strong and whole the cloves were',
  pepper:     'the sharpness and quality of the black pepper',
  cardamom:   'the intense aroma of the green cardamom',
};
...
function buildAspectSentence(liked) {
  ...
  const OPS = ['I especially appreciated ', 'I particularly liked ', 'What stood out most was ', 'I was genuinely impressed by ', 'A real highlight was '];
...
function polishManualZoffReview(rawText, liked, shopName) {
  ...
  text = text
    .replace(/\bwas good\b/gi, 'was genuinely good')
    .replace(/\bis good\b/gi, 'is very good')
    .replace(/\bvery very\b/gi, 'very')
    .replace(/\bnice taste\b/gi, 'great taste')
    .replace(/\bgud\b/gi, 'good')
    .replace(/\btastey\b/gi, 'tasty')
    .replace(/\bzoff\b/gi, 'ZOFF')
    .replace(/\s{2,}/g, ' ')
    .trim();

  const asp = buildAspectSentence(liked);
  const name = shopName || 'ZOFF Foods';

  const intros = [
    `I recently ordered from ${name} and wanted to share my experience.`,
    `Ordered from ${name} online and I'm glad to share how it went.`,
    `I tried ${name}'s spices recently and it was a genuinely good experience.`,
    `Picked up a few things from ${name} and thought I'd leave a quick review.`,
  ];
  const closings = [
...
export default function ZoffReviewPage() {
  ...
  // Self-provision the ZOFF row in Supabase on first load (no admin step needed)
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from('businesses').select('*').eq('id', ZOFF_ID).single();
      if (data) {
        setShop(data);
        await supabase.from('businesses').update({ scans: (data.scans || 0) + 1 }).eq('id', ZOFF_ID);
        return;
      }
      const { data: inserted, error: insertError } = await supabase
        .from('businesses').insert([ZOFF_FALLBACK]).select().single();
      setShop(insertError ? ZOFF_FALLBACK : inserted);
    })();
  }, []);
  ...
  const googleUrl = shop?.google_profile_url || ZOFF_FALLBACK.google_profile_url;

  const handleGenerate = async () => {
    setLoading(true);
    await new Promise((r) => setTimeout(r, 900));
    const { data: fresh } = await supabase.from('businesses').select('reviews_generated').eq('id', ZOFF_ID).single();
    const currentVariant = fresh?.reviews_generated ?? shop?.reviews_generated ?? 0;

    const result = generateShopReview({
      shopName: shop?.shop_name || 'ZOFF Foods',
      ownerName: '',
      businessType: 'spices',
      subType: shop?.sub_type || '',
      location: shop?.location || 'Bengaluru',
    ...
    setLoading(false);
    setStep(6);
    await supabase.from('businesses').update({ reviews_generated: currentVariant + 1 }).eq('id', ZOFF_ID);
  };
  ...
  const handlePolish = async () => {
    ...
    setPolishedReview(polishManualZoffReview(manualText, liked, shop?.shop_name));
    setPolishing(false);
    setStep(6);
    const { data: fresh } = await supabase.from('businesses').select('reviews_generated').eq('id', ZOFF_ID).single();
    await supabase.from('businesses').update({ reviews_generated: (fresh?.reviews_generated || 0) + 1 }).eq('id', ZOFF_ID);
  };
  ...
  if (!shop) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fdf6ec' }}>
      <div style={{ width: 40, height: 40, border: '4px solid #f5cba7', borderTopColor: THEME.primary, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      <style>{ANIM}</style>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#fdf6ec 0%,#fef3e7 50%,#fff8f0 100%)', padding: '0 0 calc(80px + env(safe-area-inset-bottom))', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
      ...
      <div style={{ background: THEME.gradient, padding: '0 20px', paddingTop: 'env(safe-area-inset-top)' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', padding: '28px 0 32px', textAlign: 'center' }}>
          ...
          <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 800, margin: '0 0 6px', lineHeight: 1.2 }}>ZOFF Foods</h1>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.18)', borderRadius: 100, padding: '5px 14px', fontSize: 12, color: 'rgba(255,255,255,0.92)', fontWeight: 600, maxWidth: '100%' }}>
            ...
            <span>Verified Brand · Zone of Fresh Foods</span>
          </div>
        </div>
      </div>

      <div style={{ height: 4, background: '#f5e3d0' }}>
        <div style={{ height: 4, background: THEME.gradient, width: `${(Math.min(step, 6) / 6) * 100}%`, transition: 'width 0.5s cubic-bezier(0.4,0,0.2,1)' }} />
      </div>

      <div style={{ maxWidth: 560, margin: '0 auto', padding: '28px 16px 0' }}>

        {step === 1 && (
          <div className="anim-up review-card" style={{ background: '#fff', borderRadius: 24, padding: 28, boxShadow: '0 8px 40px rgba(192,57,43,0.1)', border: '1px solid #f5e3d0' }}>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              <div style={{ fontSize: 40, marginBottom: 10 }}>⭐</div>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', margin: 0 }}>How was your order?</h2>
              <p style={{ color: '#64748b', fontSize: 14, marginTop: 6 }}>Your feedback helps other shoppers.</p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {RATING_OPTIONS.map((r) => (
                <button key={r.key} onClick={() => { setRating(r.key); setStep(2); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', borderRadius: 16, border: `2px solid ${rating === r.key ? THEME.primary : '#e2e8f0'}`, background: rating === r.key ? THEME.light : '#fafafa', cursor: 'pointer', textAlign: 'left' }}>
        ...
        {step === 2 && (
          <div className="anim-up review-card" style={{ background: '#fff', borderRadius: 24, padding: 28, boxShadow: '0 8px 40px rgba(192,57,43,0.1)', border: '1px solid #f5e3d0' }}>
              ...
              {LIKED_OPTIONS.map((opt) => {
                ...
                return (
                  <button key={opt.key} onClick={() => toggleLiked(opt.key)}
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 14, border: `2px solid ${sel ? THEME.primary : '#e2e8f0'}`, background: sel ? THEME.light : '#fafafa', cursor: 'pointer', textAlign: 'left', fontSize: 13, fontWeight: sel ? 700 : 500, color: sel ? THEME.dark : '#374151' }}>
            ...
            <button onClick={() => setStep(3)} disabled={liked.length === 0}
              style={{ width: '100%', padding: '15px', borderRadius: 14, background: liked.length ? THEME.gradient : '#e2e8f0', color: '#fff', border: 'none', fontWeight: 700, fontSize: 15, cursor: liked.length ? 'pointer' : 'not-allowed' }}>
        ...
        {step === 3 && (
          <div className="anim-up review-card" style={{ background: '#fff', borderRadius: 24, padding: 28, boxShadow: '0 8px 40px rgba(192,57,43,0.1)', border: '1px solid #f5e3d0' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', margin: 0 }}>How long have you been buying?</h2>
              <p style={{ color: '#64748b', fontSize: 13, marginTop: 6 }}>Helps personalise your review.</p>
            </div>
            <div className="dur-grid" style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
              {DURATION_OPTIONS.map((opt) => {
                const sel = duration === opt.key;
                return (
                  <button key={opt.key} onClick={() => setDuration(opt.key)}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderRadius: 14, border: `2px solid ${sel ? THEME.primary : '#e2e8f0'}`, background: sel ? THEME.light : '#fafafa', cursor: 'pointer', textAlign: 'left', fontSize: 14, fontWeight: sel ? 700 : 500, color: sel ? THEME.dark : '#374151' }}>
                    <span>{opt.label}</span>
            ...
            <button onClick={() => setStep(4)} disabled={!duration}
              style={{ width: '100%', padding: '15px', borderRadius: 14, background: duration ? THEME.gradient : '#e2e8f0', color: '#fff', border: 'none', fontWeight: 700, fontSize: 15, cursor: duration ? 'pointer' : 'not-allowed' }}>
              Continue →
            </button>
          </div>
        )}
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <button onClick={() => setMode('auto')}
                    style={{ padding: '20px 22px', borderRadius: 18, border: '2px solid #f5e3d0', background: THEME.light, cursor: 'pointer', textAlign: 'left' }}>
                  ...
                  <div className="type-row" style={{ display: 'flex', gap: 10 }}>
                    {REVIEW_TYPES.map((t) => (
                      <button key={t.key} onClick={() => setReviewType(t.key)}
                        style={{ flex: 1, padding: '12px 8px', borderRadius: 14, border: `2px solid ${reviewType === t.key ? THEME.primary : '#e2e8f0'}`, background: reviewType === t.key ? THEME.light : '#fafafa', cursor: 'pointer' }}>
            ...
            {mode === 'manual' && (
              <>
                ...
                <textarea
                  ...
                  placeholder="Write your experience here... (what you ordered, what stood out, how it tasted)"
          ...
          <div className="anim-up">
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <p style={{ color: '#64748b', fontSize: 14 }}>
                {mode === 'manual' ? 'Polished and structured for you.' : 'Copy it and paste on Google.'}
              </p>
            </div>

            <div className="review-card" style={{ background: '#fff', borderRadius: 24, padding: 28, boxShadow: '0 8px 40px rgba(192,57,43,0.12)', border: '1px solid #f5e3d0', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: THEME.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 16 }}>Z</div>
                <div>
