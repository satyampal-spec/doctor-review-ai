...
export default function AltiusCampPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f6f8fc', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Header */}
      <div style={{ background: THEME.gradient, padding: '20px 16px 26px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 420, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', background: '#fff', padding: '7px 16px', borderRadius: 14, marginBottom: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}>
            <img src="/camp/altius-logo.webp" alt="Altius Hospitals" style={{ height: 30, display: 'block' }} />
          </div>
          <div style={{ fontSize: 'clamp(22px, 8vw, 32px)', fontWeight: 900, color: '#fff', lineHeight: 1.2, textShadow: '0 2px 12px rgba(0,0,0,0.15)' }}>
            🎉 {OFFER_TITLE}
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: '0 16px' }}>
        <div style={{ maxWidth: 420, margin: '0 auto', background: '#fff', borderRadius: 18, boxShadow: '0 12px 40px rgba(242,96,12,0.18)', padding: '20px 20px 22px', marginTop: -14 }}>
          {!submitted ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: THEME.navy, marginBottom: 3 }}>⏳ Limited slots for today</div>
                <div style={{ fontSize: 12.5, color: '#64748b' }}>Leave your details and our team will call you right back</div>
              </div>

              <form onSubmit={handleSubmit}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#374151', marginBottom: 5 }}>Your Name</label>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Enter your name"
                  style={{ width: '100%', boxSizing: 'border-box', padding: '13px 16px', borderRadius: 13, border: `2px solid ${THEME.ring}`, fontSize: 16, marginBottom: 14, outline: 'none' }}
                  onFocus={(e) => (e.target.style.borderColor = THEME.primary)}
                  onBlur={(e) => (e.target.style.borderColor = THEME.ring)}
                />

                <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#374151', marginBottom: 5 }}>Phone Number</label>
                <input
                  value={phone}
                  onChange={(e) => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                  placeholder="10-digit mobile number"
                  inputMode="numeric"
                  style={{ width: '100%', boxSizing: 'border-box', padding: '13px 16px', borderRadius: 13, border: `2px solid ${THEME.ring}`, fontSize: 16, marginBottom: 18, outline: 'none' }}
                  onFocus={(e) => (e.target.style.borderColor = THEME.primary)}
                  onBlur={(e) => (e.target.style.borderColor = THEME.ring)}
                />
        ...
        <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 11.5, margin: '14px 0 20px' }}>
