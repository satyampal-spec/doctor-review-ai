...
// Seeded picker — same engine as shopReviewGenerator
// variant = reviews_generated count; guarantees review #N ≠ review #N-1
...
const EN_SPECIALTY = {
  ...
  'Dermatologist': [
    "My skin issue was correctly diagnosed on the first visit — no trial and error.",
    "The treatment plan for my skin condition was clear, practical, and it actually worked.",
    "Explained exactly what was causing the issue and how the treatment would help.",
    "No unnecessary products pushed — just a straightforward, effective skincare treatment plan.",
  ],
  'Orthopedic Surgeon': [
    "My joint and bone issue was diagnosed accurately and the recovery plan worked exactly as explained.",
    "Physiotherapy and treatment advice was practical and genuinely helped my mobility.",
    "Explained the X-rays and scans in a way I could actually understand.",
    "No rush toward surgery — a sensible, staged treatment plan was suggested first.",
  ],
  'ENT Specialist': [
    "My ear, nose and throat issue was cleared up quickly, with a clear explanation of what was going on.",
    "A sinus problem that had been bothering me for months was finally properly diagnosed and treated.",
    "A thorough ear, nose and throat examination — nothing felt rushed.",
    "Finally got real relief from a recurring throat issue that other doctors had only patched up.",
  ],
  'Neurologist': [
    "Took my symptoms seriously and investigated thoroughly instead of dismissing them.",
    "Explained a complicated neurological issue in terms I could actually follow.",
    "The treatment plan was carefully explained, including what to expect at each stage.",
  ],
  'Psychiatrist': [
    "Made a difficult conversation feel completely safe and judgment-free.",
    "Took the time to really understand what I was going through before suggesting anything.",
    "The treatment plan was explained clearly, with realistic expectations set from the start.",
  ],
  'Other': [
    "Diagnosed the issue accurately and explained the treatment plan clearly.",
    "Handled my case with genuine expertise and took the time to explain everything.",
    "Thorough, attentive, and clearly knowledgeable in their field.",
  ],
};
...
const KN_SPECIALTY = {
  ...
  'Dermatologist': [
    "Nanna skin problem modalane visit alliye sariyagi diagnose aaytu.",
    "Skin condition ge treatment plan clear agittu, adhu kelsa kuda maaditu.",
    "Yenu problem, yaake bantu antha clearly explain maadidru.",
  ],
  'Orthopedic Surgeon': [
    "Joint mattu bone problem sariyagi diagnose aaytu, recovery plan explain maadidhange kelsa maaditu.",
    "Physiotherapy advice practical aagittu, mobility improve aaytu.",
    "X-ray, scans ella arthavaaguva hage explain maadidru.",
  ],
  'ENT Specialist': [
    "Ear, nose, throat problem quickly clear aaytu — yenu aagide antha sariyagi explain maadidru.",
    "Kelavu tingalinda irodha sinus problem finally sariyagi diagnose maadi treat maadidru.",
    "Ear, nose, throat ella thorough agi check maadidru — rush maadlilla.",
  ],
  'Neurologist': [
    "Nanna symptoms serious aagi thagondu thorough agi investigate maadidru.",
    "Complicated neurological issue anu simple aagi explain maadidru.",
  ],
  'Psychiatrist': [
    "Kashtavaada maathukathe safe aagi, judgment illlada feel aaytu.",
    "Nanna problem ninnu arthamaadikoLLoke time tegedukondru, mele suggest maadidru.",
  ],
  'Other': [
    "Problem sariyagi diagnose maadi treatment plan clear agi explain maadidru.",
    "Thorough agi, knowledgeable agi handle maadidru nanna case anu.",
  ],
};
...
function assembleReview(lang, vars, rating, liked, duration, variant = 0) {
  const r = rating || 'excellent';
  const d = duration || 'first';
  const langOffset = lang === 'kn' ? 500 : 0;
  const seededPick = makeSeededPick(variant + langOffset);

  const opening = fill(seededPick((lang.openings[r] || lang.openings.excellent)[d] || lang.openings.excellent.first), vars);
  ...
  // Bug fix: previously fell back to 'General Physician' content (which says "GP")
  // for any specialization not explicitly listed above — e.g. an ENT review would
  // wrongly get GP-flavoured lines. Fall back to the generic 'Other' pool instead,
  // which never mentions GP/specialization-specific claims.
  const specPool = lang.specialty[vars.specialization] || lang.specialty['Other'] || [''];
  const specSentence = fill(seededPick(specPool), vars);
  const closing = fill(seededPick((lang.closings[r] || lang.closings.excellent)[d] || lang.closings.excellent.first), vars);
  const bridge = fill(seededPick(lang.bridges[d] || lang.bridges.first), vars);
...
export function generateReview({ doctorName, clinicName, specialization, location, rating, liked, duration, variant = 0 }) {
  ...
  const variantBoost = Math.min(3, Math.floor(variant / 10));
  const scores = {
  ...
  return {
    reviews: {
      english: assembleReview(en, vars, rating, liked, duration, variant),
      kannada: assembleReview(kn, vars, rating, liked, duration, variant),
    },
    scores,
    variant,
  };
