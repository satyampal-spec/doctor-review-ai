...
export const CATEGORY_CONFIG = {
  hospital: {
    ...
    emoji: '🏥',
    icon: '⚕️',
    subTypes: ['Multi-specialty', 'General Hospital', 'Dental Hospital', 'Eye Hospital', 'Orthopaedic', 'Maternity Hospital', "Children's Hospital", 'Cardiac Hospital'],
    nameLabel: 'Hospital Name',
    ownerLabel: 'Doctor / Administrator Name',
    reviewPath: 'hospital', // uses /review/hospital/[id] instead of /review/shop/[id]
    likedOptions: [
    ...
    durationOptions: [
      ...
      { key: 'regular',    label: 'Regular patient' },
      { key: 'emergency',  label: 'Emergency visit' },
    ],
  },
  clinic: {
    ...
    emoji: '🩺',
    icon: '⚕️',
    subTypes: ['General Clinic', 'Dental Clinic', 'Skin Clinic', 'Eye Clinic', 'ENT Clinic', 'Physiotherapy Clinic', 'Diagnostic & Lab Center', 'Multi-specialty Clinic', 'Ayurvedic Clinic', 'Homeopathy Clinic'],
    nameLabel: 'Clinic Name',
    ownerLabel: 'Doctor / Administrator Name',
    reviewPath: 'clinic', // uses /review/clinic/[id], auto-generate + write-your-own-and-polish, like hospitals
    likedOptions: [
      { key: 'experienced_doctor',   label: 'Experienced doctor',         emoji: '🩺' },
      { key: 'quick_diagnosis',      label: 'Quick & accurate diagnosis', emoji: '🎯' },
      { key: 'clean_hygienic',       label: 'Clean & hygienic',           emoji: '✨' },
      { key: 'short_wait',           label: 'Short waiting time',         emoji: '⏱️' },
      { key: 'caring_staff',         label: 'Caring & friendly staff',    emoji: '💙' },
      { key: 'affordable',          label: 'Affordable fees',            emoji: '💰' },
      { key: 'clear_communication', label: 'Clear explanation',          emoji: '💬' },
      { key: 'good_followup',       label: 'Good follow-up care',        emoji: '📋' },
      { key: 'easy_appointment',    label: 'Easy appointment booking',   emoji: '📅' },
      { key: 'modern_equipment',    label: 'Modern equipment',           emoji: '🔬' },
      { key: 'no_unnecessary_tests',label: 'No unnecessary tests',       emoji: '🚫' },
      { key: 'comfortable',         label: 'Comfortable environment',    emoji: '🛋️' },
    ],
    durationOptions: [
      { key: 'first', label: 'First visit' },
      { key: 'few_times', label: 'Visited a few times' },
      { key: 'regular', label: 'Regular patient (months)' },
      { key: 'loyal', label: 'Family clinic for years' },
    ],
  },
  clothes: {
    label: 'Clothes Shop',
  ...
  spices: {
    label: 'Spices & Masala Brand',
    emoji: '🌶️',
    icon: '🧂',
    subTypes: ['Whole Spices', 'Ground Masala & Spice Powders', 'Spice Blends', 'Ready-to-Cook Gravy & Marinades', 'Dry Fruits & Seasonings', 'Multi-category Spice Brand'],
    nameLabel: 'Brand Name',
    ownerLabel: 'Founder / Manager Name',
    reviewPath: 'spices', // uses /review/spices/[id], auto-generate + write-your-own-and-polish, like hospitals
    likedOptions: [
      { key: 'aroma',      label: 'Strong aroma, fresh smell',       emoji: '👃' },
      { key: 'purity',     label: 'Pure, no adulteration',           emoji: '✅' },
      { key: 'packaging',  label: 'Resealable zip-lock pack',        emoji: '📦' },
      { key: 'cool_grind', label: 'Not burnt / cool-ground taste',   emoji: '❄️' },
      { key: 'consistent', label: 'Consistent every order',          emoji: '🔁' },
      { key: 'value',      label: 'Good value for money',            emoji: '💰' },
      { key: 'delivery',   label: 'Fast Zepto/Blinkit delivery',     emoji: '⚡' },
      { key: 'clean',      label: 'No stones/husk/dust',             emoji: '🧹' },
      { key: 'cumin',      label: 'Cumin (Jeera) quality',           emoji: '🟤' },
      { key: 'soya',       label: 'Soya Chunks quality',             emoji: '🍖' },
      { key: 'clove',      label: 'Cloves (Laung) quality',          emoji: '🌰' },
      { key: 'pepper',     label: 'Black Pepper quality',            emoji: '⚫' },
      { key: 'cardamom',   label: 'Green Cardamom quality',          emoji: '💚' },
    ],
    durationOptions: [
      { key: 'first', label: 'First order' },
      { key: 'few_times', label: 'Ordered a few times' },
      { key: 'regular', label: 'Regular buyer (months)' },
      { key: 'loyal', label: 'Long-time / family staple' },
    ],
  },
};
...
const SPICES_EN = {
  openings: {
    excellent: {
      first: [
        "Ordered from {shop} for the first time on Zepto and was genuinely impressed.",
        "First time trying {shop} and I'm glad I did, didn't expect this level of quality from a quick-commerce order.",
        "Picked this up on a whim while ordering groceries online and it turned out to be one of the better spice purchases I've made.",
        "Tried {shop} for the first time after seeing it recommended online, and it's lived up to it.",
        "First order from {shop} and it's already replaced what I used to buy from the local store.",
        "Was skeptical about ordering spices online, but {shop} changed my mind on the first order itself.",
      ],
      few_times: [
        "This is my second or third order from {shop} and the quality has stayed consistent.",
        "Ordered from {shop} a couple of times now and I keep coming back to it.",
        "A few orders in, and {shop} has become my default choice for spices.",
      ],
      regular: [
        "I've been ordering from {shop} for a few months now and it's become a kitchen staple.",
        "Regular buyer here, reordering spices from {shop} every month without a second thought.",
        "Months of ordering from {shop} and the quality has never dipped once.",
      ],
      loyal: [
        "Been buying from {shop} for a long time now and it remains a staple in our kitchen.",
        "Spices from {shop} have been part of our monthly grocery order for a long time now, never disappointed.",
        "Long-time customer here, {shop} is one brand I never switch out.",
      ],
    },
    good: {
      first: ["Good first order, quality was solid and matched what was described.", "Solid spice brand. Fresh and reasonably priced."],
      few_times: ["Consistently good across orders.", "Reliable every time I order."],
      regular: ["Regular buyer, no complaints, dependable quality."],
      loyal: ["Years of good experience with {shop}."],
    },
    average: {
      first: ["Decent, got what I ordered."],
      few_times: ["Fine across a few orders."],
      regular: ["Adequate for regular use."],
      loyal: ["Familiar and consistent for basic needs."],
    },
  },
  aspects: {
    aroma: [
      "The aroma the moment you open the pack is the first thing that gets you, strong, not faint.",
      "Even after a few weeks of use, it still smells fresh, not flat like some packets I've had before.",
      "You can genuinely smell the difference the moment you open a {shop} pack, compared to ones that have been sitting around.",
    ],
    purity: [
      "No added colour, no odd chemical smell, just clean, honest spices.",
      "The pack clearly mentions lab testing and FSSAI compliance, which gave me more confidence buying online.",
      "In a category where adulteration is a real worry, it's reassuring to actually see and smell what you're buying from {shop}.",
    ],
    packaging: [
      "The zip-lock pouch is genuinely useful, I don't have to transfer it into a separate steel dabba the moment I open it.",
      "Stands up on its own on the kitchen shelf and reseals properly, small thing but it matters daily.",
      "The packaging keeps things fresh between uses, unlike the flimsy plastic packets I've dealt with before.",
    ],
    cool_grind: [
      "You can tell the grinding doesn't burn the spice, the smell is earthy and alive, not dusty.",
      "None of that 'cooked' smell you sometimes get from cheaper ground masalas.",
    ],
    consistent: [
      "Ordered from {shop} a few times now and the quality has been exactly the same every single time.",
      "No batch-to-batch difference that I've noticed, consistent aroma and colour across multiple orders.",
    ],
    value: [
      "A little more than the mass-market brands, but the quality difference justifies the price.",
      "Good value for the quality, doesn't feel like you're paying extra just for the brand name.",
    ],
    delivery: [
      "Ordered on Zepto and it showed up within minutes, properly sealed and not squashed in transit.",
      "Quick delivery through Blinkit/Zepto and the packaging arrived intact.",
    ],
    clean: [
      "No stones, husk, or plant debris mixed in, something that is unfortunately common with loose spices.",
      "Everything in the pack looked clean and properly sorted before packing.",
    ],
    cumin: [
      "The cumin seeds have a strong, nutty aroma the moment you open the pack, nothing like the flat smell you get from a packet that's been sitting on a shelf for months.",
      "Dry-roasted a spoon of the jeera for tempering and the whole kitchen smelled of it within seconds, that's how you know it's fresh.",
      "No broken seeds, no dust at the bottom, just clean, plump cumin seeds.",
      "The roasted cumin powder has an intense, nutty kick that regular jeera powder just doesn't have.",
    ],
    soya: [
      "The soya chunks turn soft and spongy after just a few minutes in hot water, no long soaking needed.",
      "Good protein content and it actually shows in the texture, these hold their shape well in curry and don't turn mushy.",
      "No strange smell straight out of the pack, which is usually my biggest complaint with soya chunks from other brands.",
      "Clean ingredient list, just soya and nothing else, and you can tell from the taste.",
    ],
    clove: [
      "The cloves are whole and plump, not the shrivelled, broken ones I've gotten from loose masala shops before.",
      "Even two or three cloves are enough to flavour a whole pot of biryani, that's how strong they are.",
      "No stems mixed in, no dust, just clean whole cloves.",
      "The pungent aroma the moment you bite into one tells you these are fresh, not sitting in a warehouse for a year.",
    ],
    pepper: [
      "The peppercorns are plump and heavy, not the shrunken, hollow ones you sometimes find in cheaper packs.",
      "Ground fresh and the sharpness hits immediately, nothing like pre-ground pepper that's gone flat.",
      "No stones, husk, or plant debris, just clean peppercorns, which isn't something I can say for every brand.",
      "A little goes a long way, the heat and aroma are both far stronger than what I was used to.",
    ],
    cardamom: [
      "The pods are full and green, not the pale, empty ones that give you nothing when you crush them.",
      "The aroma when you crush a pod is intense, fills the kitchen in seconds.",
      "No empty husks in the pack, every single pod actually has seeds inside.",
      "A little more expensive than loose elaichi, but the aroma justifies it completely.",
    ],
  },
  bridges: {
    first: [
      "For a first order, {shop} set a standard I now judge other spice brands by.",
      "I went in with average expectations and {shop} exceeded them from the first pack.",
    ],
    few_times: [
      "What I've noticed across orders is that the quality and aroma stay consistent.",
      "Each order confirms exactly why I keep coming back to {shop}.",
    ],
    regular: [
      "Months of ordering and the standard has not dropped once.",
      "The consistency over time is what makes {shop} my default choice.",
    ],
    loyal: [
      "Years of trust, and nothing has happened to break it.",
      "Long-term customers like me keep reordering {shop} for a reason.",
    ],
  },
  closings: {
    excellent: {
      first: [
        "Would genuinely recommend {shop} to anyone looking for a reliable spice brand.",
        "Already reordered and will keep coming back for this.",
        "Highly recommend, worth the switch.",
        "Five stars, no hesitation.",
      ],
      few_times: ["Will keep reordering, no reason to switch brands now.", "My go-to for spices from here on."],
      regular: ["Month after month, still my first choice. Recommend without hesitation.", "My consistent go-to, recommend to anyone stocking their kitchen."],
      loyal: ["Years in and still my first recommendation. That says it all.", "The whole family cooks with {shop} now. No intention of switching."],
    },
    good: {
      first: ["Will order again. Good brand.", "A solid choice for spices."],
      few_times: ["Consistent and reliable, recommend.", "Good experience each order."],
      regular: ["A dependable regular choice."],
      loyal: ["Trusted over the years. Recommend."],
    },
    average: {
      first: ["Fine for basic needs."], few_times: ["Adequate for regular use."], regular: ["Fine for ongoing purchases."], loyal: ["A familiar fallback."],
    },
  },
};
...
const CLINIC_EN = {
  openings: {
    excellent: {
      first: [
        "First visit to {shop} in {location} and I left genuinely impressed.",
        "I had been putting off seeing a doctor for weeks, finally came to {shop} and I'm glad I did.",
        "A friend recommended {shop} and the recommendation was spot on.",
        "I came to {shop} with low expectations and left completely reassured.",
        "I have been to a few clinics in {location}, {shop} stands out.",
        "Walked into {shop} without knowing what to expect and left very satisfied.",
        "First time at {shop} and I already know I'll be coming back.",
        "I had seen other clinics in {location} before, wish I had come to {shop} first.",
      ],
      few_times: [
        "I have been to {shop} a few times now and the quality has been consistent every visit.",
        "Each visit to {shop} confirms why I chose it in the first place.",
        "A couple of visits in and {shop} is already my go-to clinic in {location}.",
      ],
      regular: [
        "I have been visiting {shop} for several months now and remain consistently satisfied.",
        "Regular visits to {shop} and the quality has never dropped.",
        "Month after month, {shop} keeps delivering the same standard of care.",
      ],
      loyal: [
        "My entire family has been visiting {shop} for years, we wouldn't go anywhere else.",
        "Years of visits and {shop} has never given us a reason to switch.",
        "Our family's trusted clinic in {location} for years now.",
      ],
    },
    good: {
      first: ["Good first visit at {shop}, got what I needed.", "Solid experience at {shop}, no complaints."],
      few_times: ["Consistently good across a few visits.", "Reliable each time I visit {shop}."],
      regular: ["Regular visits for months, dependable."],
      loyal: ["Years of good experiences at {shop}."],
    },
    average: {
      first: ["Decent first visit, covered the basics."],
      few_times: ["Fine across a few visits."],
      regular: ["Adequate for routine needs."],
      loyal: ["Familiar and consistent over the years."],
    },
  },
  aspects: {
    experienced_doctor: [
      "The doctor at {shop} is clearly experienced, diagnosed the issue quickly and correctly.",
      "Years of practice show, the doctor asked the right questions and got to the point fast.",
      "Not the kind of rushed consultation you get elsewhere, genuinely thorough.",
    ],
    quick_diagnosis: [
      "The diagnosis was quick and accurate, no unnecessary back and forth.",
      "First visit, correct diagnosis, did not need to come back for corrections.",
      "Sharp clinical thinking, got to the problem without overcomplicating anything.",
    ],
    clean_hygienic: [
      "The clinic is clean and well-maintained, you notice it the moment you walk in.",
      "Proper hygiene throughout, gives confidence from the start.",
      "A tidy, professional space that reflects the quality of care inside.",
    ],
    short_wait: [
      "Minimal waiting time compared to other clinics I've been to in {location}.",
      "My appointment was on time, in and out without wasting half my day.",
      "The clinic actually runs on schedule, which is rare.",
    ],
    caring_staff: [
      "The staff at {shop} are friendly, patient, and genuinely helpful.",
      "From reception to the consultation room, everyone was courteous and organised.",
      "Small thing, but the staff being warm and welcoming makes a real difference.",
    ],
    affordable: [
      "The consultation fee is fair for the quality of care received.",
      "Good value for money, no hidden charges, no surprise add-ons.",
      "Affordable without cutting corners on the quality of treatment.",
    ],
    clear_communication: [
      ...
      "I was told what each medicine does and why, that clarity made all the difference.",
    ],
    good_followup: [
      "The follow-up was just as attentive as the first visit, no drop in quality.",
      "I got a follow-up check-in, which I wasn't expecting but really appreciated.",
      "Post-visit care was handled well, I felt looked after beyond just the appointment.",
    ],
    easy_appointment: [
      "Booking an appointment was quick and hassle-free.",
      "No long hold times or confusing process, scheduling was smooth.",
      "Easy to get a slot even on short notice.",
    ],
    modern_equipment: [
      "The clinic is equipped with proper, modern diagnostic equipment.",
      "Nothing felt outdated, the equipment used was clearly well-maintained.",
      "Tests were done on-site with modern equipment, saving me a trip elsewhere.",
    ],
    no_unnecessary_tests: [
      "I wasn't sent for a dozen tests before getting a diagnosis, just the necessary ones.",
      "No over-prescription, no unnecessary investigations, practical, honest medicine.",
      "I appreciated that the approach here is to treat what needs treating, nothing more.",
    ],
    comfortable: [
      "The waiting area is comfortable and calm, not the usual chaotic clinic experience.",
      "A relaxed environment that actually puts you at ease before your consultation.",
      "Clean, comfortable, and well-organised, a pleasant place to be even when unwell.",
    ],
  },
  bridges: {
    first: [
      "What stood out most on this first visit was how unhurried and attentive the whole consultation was.",
      "I came in with low expectations. They were exceeded on every count.",
      "It is rare that a first visit to a clinic leaves you feeling this confident.",
    ],
    few_times: [
      "What I've noticed across these visits is that the quality is consistent every single time.",
      "Each visit reinforces exactly why I keep coming back to {shop}.",
    ],
    regular: [
      "Months of visits and the standard has not dropped once.",
      "The consistency over time is what makes {shop} my default choice in {location}.",
    ],
    loyal: [
      "Years of visits across the whole family and the trust has only grown.",
      "When a clinic earns the trust of an entire family over time, that is not by accident.",
    ],
  },
  closings: {
    excellent: {
      first: [
        "Already planning my next visit. Highly recommend {shop} in {location}.",
        "If you're looking for a good clinic in {location}, this is the one.",
        "Would recommend without hesitation to anyone nearby.",
      ],
      few_times: ["Consistently good, will keep coming back.", "Reliable and genuinely caring. Recommend to anyone nearby."],
      regular: ["Still my first recommendation when anyone in {location} asks for a clinic.", "No plans to go anywhere else."],
      loyal: ["My whole family vouches for {shop}, the highest recommendation I can give.", "Years of trust, never broken."],
    },
    good: {
      first: ["Will return. Good overall experience.", "Recommend for anyone needing a solid clinic in {location}."],
      few_times: ["Consistent and reliable, that's what matters."],
      regular: ["A dependable regular choice in {location}."],
      loyal: ["Trusted over the years. Recommend."],
    },
    average: {
      first: ["Adequate for routine needs."],
      few_times: ["Fine for regular follow-ups."],
      regular: ["Reliable for ongoing basic care."],
      loyal: ["Handles general health needs for the family."],
    },
  },
};
...
const KN_TEMPLATES = {
  ...
  spices: {
    openings: {
      excellent: {
        first: [
          "{shop} inda first order Zepto alli maadide, thumba impressed aade.",
          "First time {shop} try maadide, expect maadirla ee quality quick-commerce order alli.",
          "Grocery order maadthiddaaga random agi tegedukonde, best spice purchase aayithu.",
          "Online spices order maadoke sceptical aagthu, {shop} first order alle mind change aaythu.",
        ],
        few_times: ["Eradu moru order aagide {shop} inda, quality consistent.", "Kelavu order aagide, {shop} default choice aagide."],
        regular: ["Kele tingalinda {shop} inda order maadthidini, kitchen staple aagide.", "Regular buyer, tingala order maadthini, second thought illlada."],
        loyal: ["Varshagalinda {shop} inda tegedukonthidini, kitchen alli staple.", "Long-time customer, {shop} switch maadalla yavaglu."],
      },
      good: {
        first: ["Good first order, quality solid.", "Solid spice brand."], few_times: ["Consistent."], regular: ["Reliable."], loyal: ["Trusted."],
      },
      average: { first: ["Decent."], few_times: ["OK."], regular: ["Adequate."], loyal: ["Familiar."] },
    },
    aspects: {
      aroma: ["Pack open maadidha kshana aroma strong agi bartade, faint alla.", "Weeks aagidru fresh vaasane iruttade, flat alla bere packets hage.", "{shop} pack open maadidaaga vaasane clearly gottaguttade."],
      purity: ["Colour add maadirla, chemical vaasane illlada, clean spices.", "Lab testing, FSSAI compliance pack mele ide, confidence sikkithu online order madoke.", "Adulteration concern illlada {shop} inda tegedukonda mele."],
      packaging: ["Zip-lock pouch useful, separate dabba ge transfer madoke beku aagilla.", "Kitchen shelf mele stand aaguthe, reseal aaguthe properly.", "Packaging freshness maintain maadutthe uses naduve."],
      cool_grind: ["Grinding spice burn maadirla antha gottagutthe, vaasane earthy.", "Cooked vaasane illlada cheaper masala hage."],
      consistent: ["Kelavu order aagide {shop} inda, quality exact same every time.", "Batch to batch difference illlada, consistent aroma, colour."],
      value: ["Mass-market brands ge hollistu jaasti, adre quality difference justify maaDutthe.", "Value for money, brand name ge extra pay maadthidini antha feel aagalla."],
      delivery: ["Zepto alli order maadide, minutes alli bandithu, sealed properly.", "Blinkit/Zepto quick delivery, packaging intact aagithu."],
      clean: ["Stones, husk, plant debris illlada, loose spices alli common issue idhu.", "Pack alli ella clean agi sorted agi ithu."],
      cumin: ["Jeera seeds strong nutty aroma, pack open maadidha kshana.", "Tempering ge roast maadide, kitchen full vaasane aaythu.", "Broken seeds illlada, dust illlada, clean cumin."],
      soya: ["Soya chunks hot water alli minutes alli soft, spongy aaguthe.", "Protein content good, texture curry alli hold aaguttade.", "Strange smell illlada pack inda straight."],
      clove: ["Cloves whole, plump, shrivelled alla loose masala shops inda sikkodu hage.", "2-3 cloves saaku biryani full pot ge flavour ge.", "Stems illlada, dust illlada, clean whole cloves."],
      pepper: ["Peppercorns plump, heavy, shrunken hollow alla.", "Fresh ground, sharpness immediately hit aaguttade.", "Stones, husk illlada, clean peppercorns."],
      cardamom: ["Pods full, green, pale empty alla.", "Crush maadidaaga aroma intense, kitchen full aaguthe seconds alli.", "Empty husks illlada, every pod seeds ide."],
    },
    bridges: {
      first: ["First order ge {shop} standard set maadithu, bere brands judge maadoke ee criteria.", "Average expectation ittu, {shop} exceed maadithu first pack inda."],
      few_times: ["Order across notice maadide quality and aroma consistent ide.", "Each order confirm maaDutthe why {shop} inda tegedukonthini antha."],
      regular: ["Tingala order aagide, standard drop aagilla.", "Consistency over time, {shop} default choice aagoke reason."],
      loyal: ["Varshagala trust, break aagilla yenu.", "Long-term customers {shop} reorder maadthare reason ide antha."],
    },
    closings: {
      excellent: {
        first: ["{shop} genuinely recommend maadthini reliable spice brand beku andre.", "Already reorder maadide, matthe barthini.", "Highly recommend, switch maadoke worth ide."],
        few_times: ["Reorder maadthini, bere brand ge switch beku antha illlada.", "My go-to spices ge illinda."],
        regular: ["Month after month first choice, recommend without hesitation.", "My consistent go-to, recommend maadthini kitchen stock maadoke."],
        loyal: ["Years agide, still first recommendation. Adhu ella heLutthe.", "Family ella {shop} inda cook maadthivi ivaaga. Switch maadoke intention illlada."],
      },
      good: { first: ["Matthe order maadthini."], few_times: ["Recommend."], regular: ["Reliable."], loyal: ["Trusted."] },
      average: { first: ["Fine."], few_times: ["OK."], regular: ["Adequate."], loyal: ["Familiar."] },
    },
  },
  clinic: {
    openings: {
      excellent: {
        first: [
          "{shop} ge first time bandhe, genuinely impress aaythu.",
          "Vaara gataLe doctor hatra hogodu postpone maadthidde, {shop} ge bandu correct aaythu.",
          "Friend recommend maadidru {shop}, spot on aaythu.",
          "Kadime expectation ittu, {shop} alli completely reassure aaythu.",
        ],
        few_times: ["Kelavu bari bandhe {shop} ge, quality consistent every time.", "Already my go-to clinic in {location}."],
        regular: ["Kele tingalinda regular illi barthidini, quality drop aagilla.", "Month after month same standard illi."],
        loyal: ["Namma family varshagalinda {shop} ge bartaare, bereyalli hogodilla.", "Years agide, trust break aagilla."],
      },
      good: {
        first: ["Good first visit, beku aaddu sikkithu."], few_times: ["Consistent agi channagide."], regular: ["Reliable agi ide."], loyal: ["Years agide good experience."],
      },
      average: {
        first: ["Decent visit, basics cover aaythu."], few_times: ["Fine agide."], regular: ["Adequate agide."], loyal: ["Familiar agide varshagalinda."],
      },
    },
    aspects: {
      experienced_doctor: ["Doctor experienced agiddare, quickly correct diagnose maadidru.", "Rush maadlilla, thorough agi noDidru."],
      quick_diagnosis: ["Diagnosis quick and accurate aaythu.", "Modalane visit alli sari diagnosis, matte bandu correct maadbeku aagilla."],
      clean_hygienic: ["Clinic clean and well-maintained agide.", "Hygiene channagide, confidence bartade."],
      short_wait: ["Waiting time kadime, {location} alli bere clinics ge compare made.", "Appointment time ge kareithu, time waste aagalilla."],
      caring_staff: ["Staff friendly and helpful agiddare.", "Reception inda consultation varige ella organized."],
      affordable: ["Consultation fee fair agide quality ge.", "Hidden charges illlada, good value."],
      clear_communication: ["Ella plain language alli explain maadidru.", "Medical jargon illlada clear answers sikkithu."],
      good_followup: ["Follow-up kuda modalane visit hage attentive aaythu.", "Follow-up call bandithu, expect maadirla."],
      easy_appointment: ["Appointment book maadoke easy aaythu.", "Short notice alli kuda slot sikkithu."],
      modern_equipment: ["Modern equipment ide clinic alli.", "Tests onsite aaythu, bere kade hogbeku aagilla."],
      no_unnecessary_tests: ["Unnecessary tests kaLisalilla, beku aaddu mattre.", "Honest practice, extra prescription illlada."],
      comfortable: ["Waiting area comfortable and calm agide.", "Clean, organized, pleasant agide."],
    },
    bridges: {
      first: ["Modalane visit alli e maTTa attentive irtare antha gottirla.", "Kadime expect maadidde, exceed aaythu."],
      few_times: ["Prathi visit confirm maaDutthe why {shop} ge barthini antha."],
      regular: ["Tingala kaala, standard drop aagilla.", "Consistency {shop} anu default choice maadutthe {location} alli."],
      loyal: ["Family trust years alli build aaythu, accident alla."],
    },
    closings: {
      excellent: {
        first: ["Already next visit plan maadide. Highly recommend.", "{location} alli olle clinic beku andre illi baanni."],
        few_times: ["Consistent agide, matte barthini.", "Recommend maadthini {location} alli iruvavarige."],
        regular: ["Still {location} alli first recommendation.", "Bereyalli hogo plan illa."],
        loyal: ["Namma family ella vouch maadthare {shop} ge.", "Varshagala trust, break aagilla."],
      },
      good: { first: ["Matthe barthini."], few_times: ["Recommend."], regular: ["Reliable."], loyal: ["Trusted."] },
      average: { first: ["Fine."], few_times: ["OK."], regular: ["Adequate."], loyal: ["Familiar."] },
    },
